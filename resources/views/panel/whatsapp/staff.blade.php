...
@section('title', 'Kişisel WhatsApp Bağlantım')
@section('content')

<div class="mb-6">
    <h1 class="text-2xl font-semibold text-gray-900">Kişisel WhatsApp Bağlantım</h1>
    <p class="text-sm text-gray-500 mt-1">Kendi WhatsApp numaranızı bağlayın — size atanan randevu mesajları bu numaradan gönderilir. Bağlı değilseniz işletme numarası kullanılır.</p>
</div>
...
@if($connection && $connection->status === 'connected')
    <div class="bg-white rounded-xl border border-gray-200 p-6 mb-4">
        ...
        <form method="POST" action="{{ route('panel.whatsapp.staff.disconnect', ['tenant_slug' => $tenant->slug, 'id' => $connection->id]) }}" class="mt-3"
...
@elseif($connection && $connection->status === 'pending')
    ...
    <script>
    setInterval(() => {
        fetch('{{ route('panel.whatsapp.staff.check', ['tenant_slug' => $tenant->slug, 'id' => $connection->id]) }}')
            .then(r => r.json())
...
<div class="bg-white rounded-xl border border-gray-200 p-6">
    ...
    <div class="space-y-4">
        <form method="POST" action="{{ route('panel.whatsapp.staff.login.phone', ['tenant_slug' => $tenant->slug]) }}" class="space-y-2">
            @csrf
